...
from forms import Data, DeleteBook
...
def home():
    ...
    try:
        ...
        print(Fore.RED + f"Error fetching books for user {session.get('user_id')}: {e}")
        books = []
        flash("Error loading your books. Please try refreshing the page.", "warning")

    return render_template("index.html", title="Home - My Reading Journey", books=books, json_form=Data(), delete_form=DeleteBook())
