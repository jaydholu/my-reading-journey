...
import { Mail, Lock, User, ArrowRight, Eye, EyeOff,
  CheckCircle, BookCopy, Star, ChartColumn, ScrollText,
  Heart, ArrowDownUp
} from 'lucide-react';
...
import { validateEmail, validateUserID } from '../../utils/validators';
...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    } else if (!validateUserID(formData.user_name)) {
      ...
      >
        ...
        <div className="relative z-10 text-white text-center space-y-6 max-w-lg">
          ...
          <div className="grid grid-cols-2 gap-4 pt-8">
            {[
              { icon: BookCopy, text: 'Track Books' },
              { icon: Star, text: 'Rate & Review' },
              { icon: ChartColumn, text: 'View Stats' },
              { icon: Heart, text: 'Add Favorites' },
              { icon: ScrollText, text: 'Create Wishlists'},
              { icon: ArrowDownUp, text: 'Import / Export Book Data'},
              ...
              <motion.div
                ...
                className="glass-strong p-4 rounded-2xl text-center flex flex-col items-center gap-2"
              >
                <Icon className=" text-3xl mb-2 text-primary-500" size={32}/>
                <div className="text-sm font-medium text-dark-700 dark:text-dark-300">{text}</div>
              </motion.div>
            ))}
          </div>
        </div>
